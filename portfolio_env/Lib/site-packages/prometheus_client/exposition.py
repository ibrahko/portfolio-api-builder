...
def push_to_gateway(
    ...
    _use_gateway(
        "PUT", gateway, job, registry, grouping_key, timeout, handler, compression
    )
...
def pushadd_to_gateway(
    ...
    _use_gateway(
        "POST", gateway, job, registry, grouping_key, timeout, handler, compression
    )
...
def _compress_payload(
    ...
    if encoding == "snappy":
        if not SNAPPY_AVAILABLE:
            raise RuntimeError(
                "Snappy compression requires the python-snappy package to be installed."
            )
