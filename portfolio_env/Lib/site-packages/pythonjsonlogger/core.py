    ...
    *Added in 3.3*: `exc_info_as_array` and `stack_info_as_array` options are added.
    ...
    def __init__(
        ...
        *Added in 4.0*:
        ...
        self.rename_fields = (
            {key: rename_fields[key] for key in rename_fields}
            if rename_fields is not None
            else {}
        )
        self.static_fields = (
            {key: static_fields[key] for key in static_fields}
    ...
    def parse(self) -> List[str]:
        ...
        if isinstance(self._style, str):
            if self._style == "__sequence__":
                raise RuntimeError(
                    "Must not call parse when fmt is a sequence of strings"
                )

            if self._style == ",":
                return [
                    field.strip() for field in self._fmt.split(",") if field.strip()
                ]
